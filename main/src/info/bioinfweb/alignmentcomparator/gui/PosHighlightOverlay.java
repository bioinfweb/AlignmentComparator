...
public class PosHighlightOverlay implements ColorOverlay {
	...
	public Color getColor(AlignmentArea alignmentArea, String sequenceID, int columnIndex) {
		...
		if ((focusedArea != null) && (focusedArea != alignmentArea)) {
			...
			int start = otherTranslator.getUnalignedIndex(sequenceID, focusedArea.getSelection().getFirstColumn()).getAfterValidIndex();
			int end = otherTranslator.getUnalignedIndex(sequenceID, focusedArea.getSelection().getLastColumn()).getBeforeValidIndex();
			//TODO Check if an index was out of range
			//TODO -1 anywhere?
