...
public class SingleLineCommentPositioner implements CommentPositioner {
	...
	private int calculateLine(Comment comment, int column, int length) {
		...
		while (iterator.hasNext()) {
			...
			if (!currentComment.equals(comment)) {
				...
				int firstPos = currentComment.getAnchor().getFirstPos();
	...
	private int calculateLength(Comment comment) {
		return Math.max(comment.getAnchor().sequenceLength(),	(int)Math2.roundUp((2 * MARGIN + 
	...
	public void position(Document document) {
		...
		while (iterator.hasNext()) {
			...
			int line = calculateLine(comment, comment.getAnchor().getFirstPos(), length);
			...
			maxColumn = Math.max(maxColumn, comment.getAnchor().getFirstPos() + length);
		}
	...
	private void paintComment(AlignmentComparisonComponent comparisonComponent, Graphics2D g, Comment comment, float x, float y) {
		...
		CommentAnchor pos = comment.getAnchor();
	...
	public double getNeededHeight(AlignmentComparisonComponent comparisonComponent) {
		...
		return (data.getMaxLine() + 1) * comparisonComponent.getFirstAlignmentArea().getPaintSettings().getTokenHeight();
	}
	...
	public double getNeededLengthAfterEnd(AlignmentComparisonComponent comparisonComponent) {
		SingleLineGlobalCommentPositionerData data = getGlobalPositionerData(comparisonComponent.getDocument());
		return Math.max(0.0, (data.getMaxColumn() - comparisonComponent.getDocument().getAlignedLength()) * comparisonComponent.getFirstAlignmentArea().getPaintSettings().getTokenWidth(0));
	}
