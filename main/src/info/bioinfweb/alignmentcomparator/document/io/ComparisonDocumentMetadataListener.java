...
public class ComparisonDocumentMetadataListener implements JPhyloIOEventListener, IOConstants {
	...
	public void processEvent(JPhyloIOEventReader source, ParentEventInformation parentEvents, JPhyloIOEvent event) throws Exception {
		if (event.getType().getContentType().equals(EventContentType.ALIGNMENT) && (tokenType == null)) {  // Alignment definitions start before a token type was specified.
			...
			if (parentEvents.isParentSequence(EventContentType.META_LITERAL, EventContentType.DOCUMENT)) {
				if (PREDICATE_TOKEN_TYPE.equals(parentEvents.getDirectParent().asLiteralMetadataEvent().getPredicate().getURI())) {
					try {
			...
			else if (parentEvents.isParentSequence(EventContentType.META_LITERAL, EventContentType.ALIGNMENT)) {
